<template>
  <v-row>
    ...
    <v-col cols="10" offset="1">
      <v-card class="mb-2">
        <v-toolbar color="primary" dark>UBAH SISWA</v-toolbar>
        <v-card-text>
...
export default {
  ...
  methods: {
    async doSave() {
      ...
      if (this.$refs.form.validate()) {
        ...
        try {
          ...
          const response = await this.$axios.put(`/siswa/${this.id}`, requestData);
    ...
    async getData() {
      try {
        const response = await this.$axios.$get(`/siswa/${this.id}`);
        const { data } = response;

        this.form.nim = data.nim;
        this.form.nama_siswa = data.nama_siswa;
    ...
    async getMataKuliahOptions() {
      try {
        const response = await this.$axios.get('/siswa');

<template>
  <v-row>
    ...
    <v-col cols="10" offset="1">
      <v-card class="mb-2">
        <v-toolbar color="primary" dark>UBAH TAGIHANSPP</v-toolbar>
        <v-card-text>
...
export default {
  ...
  methods: {
    async doSave() {
      ...
      if (this.$refs.form.validate()) {
        ...
        try {
          const requestData = {
            nim: this.form.nim,
            nama_tagihan_spp: this.form.nama_tagihan_spp,
          ...
          const response = await this.$axios.put(`/tagihan-spp/${this.id}`, requestData);
    ...
    async getData() {
      try {
        const response = await this.$axios.$get(`/tagihan-spp/${this.id}`);
        const { data } = response;

        this.form.nim = data.nim;
        this.form.nama_tagihan_spp = data.nama_tagihan_spp;

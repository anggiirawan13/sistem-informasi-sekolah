<template>
  <v-row>
    ...
    <v-col cols="10" offset="1">
      <v-card class="mb-2">
        <v-toolbar color="primary" dark>UBAH TRANSAKSI</v-toolbar>
        <v-card-text>
...
export default {
  ...
  methods: {
    async doSave() {
      ...
      if (this.$refs.form.validate()) {
        ...
        try {
          ...
          const response = await this.$axios.put(`/transaksi/${this.id}`, requestData);
    ...
    async getData() {
      try {
        const response = await this.$axios.$get(`/transaksi/${this.id}`);
        const { data } = response;

        this.form.nim = data.nim;
        this.form.nama_transaksi = data.nama_transaksi;

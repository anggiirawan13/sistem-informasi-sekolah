<template>
  <v-row>
    ...
    <v-col cols="10" offset="1">
      <v-card class="mb-2">
        <v-toolbar color="primary" dark>TAMBAH KOMPONEN</v-toolbar>
        <v-card-text>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        nama_lengkap: [
          (v) => !!v || this.$t("FIELD_IS_REQUIRED", { field: "Nama Komponen" }),
        ],
      },
  ...
  methods: {
    async doSave() {
      ...
      if (this.$refs.form.validate()) {
        ...
        try {
          ...
          const response = await this.$axios.post('/komponen', requestData);
    ...
    async getMataKuliahOptions() {
      try {
        const response = await this.$axios.get('/komponen');

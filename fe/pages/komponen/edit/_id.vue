<template>
  <v-row>
    ...
    <v-col cols="10" offset="1">
      <v-card class="mb-2">
        <v-toolbar color="primary" dark>UBAH KOMPONEN</v-toolbar>
        <v-card-text>
        ...
        <v-card-actions>
          <v-btn to="/komponen" color="secondary">Back</v-btn>
          <v-spacer />
...
export default {
  ...
  data() {
    return {
      breadcrumbs: [
        { text: "Siswa", to: "/komponen", disabled: false, exact: true },
      ...
      rules: {
        ...
        nama_komponen: [
          (v) => !!v || this.$t("FIELD_IS_REQUIRED", { field: "Nama Siswa" }),
        ],
      },
  ...
  methods: {
    async doSave() {
      ...
      if (this.$refs.form.validate()) {
        ...
        try {
          const requestData = {
            nim: this.form.nim,
            nama_komponen: this.form.nama_komponen,
          ...
          const response = await this.$axios.put(`/komponen/${this.id}`, requestData);
    ...
    async getData() {
      try {
        const response = await this.$axios.$get(`/komponen/${this.id}`);
        const { data } = response;

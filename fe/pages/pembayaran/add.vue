<template>
  <v-row>
    ...
    <v-col cols="10" offset="1">
      <v-card class="mb-2">
        <v-toolbar color="primary" dark>TAMBAH PEMBAYARAN</v-toolbar>
        <v-card-text>
...
export default {
  ...
  methods: {
    async doSave() {
      ...
      if (this.$refs.form.validate()) {
        ...
        try {
          ...
          const response = await this.$axios.post('/pembayaran', requestData);
    ...
    async getMataKuliahOptions() {
      try {
        const response = await this.$axios.get('/pemayaran');
